              ...
              <article key={l.id} className={`rounded-xl border border-r-4 border-border bg-card p-4 ${priorityBorder[l.priority]}`}>
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5 font-medium text-foreground">{l.name}<TransferStar show={l.isTransferred} exhausted={l.transferredExhausted} /><TransferBadge show={l.manualTransferred} />{!isManager && <PullCountdown pull={l.pull} />}{l.stale && <span className="rounded-full bg-warning/15 px-1.5 py-0.5 text-[10px] font-bold text-warning" title="مهتم بلا متابعة من ٧ أيام">راكد</span>}</div>
