              ...
              <article key={l.id} className={`rounded-xl border border-r-4 border-border bg-card p-4 ${priorityBorder[l.priority]}`}>
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5 font-medium text-foreground">{l.name}<TransferStar show={l.isTransferred} exhausted={l.transferredExhausted} /><TransferBadge show={l.manualTransferred} was={l.wasStage} />{!isManager && <PullCountdown pull={l.pull} />}{l.staleMeta && <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-bold ${l.staleMeta.isHot ? "bg-destructive/15 text-destructive" : "bg-warning/15 text-warning"}`} title={l.staleMeta.reason === "NO_DATE" ? "بلا موعد قادم" : "فات موعده المسجّل"}>راكد {toArabicDigits(l.staleMeta.days)} يوم</span>}</div>
