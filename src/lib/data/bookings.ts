...
export async function getBookings(): Promise<BookingsData> {
  ...
  const cards: BookingCard[] = rows.map((b) => {
    ...
    return {
      ...
      nationality: mine ? b.nationality : null,
      ...
      // نص السبب وتفاصيل الخصم حقول مالية حساسة — لصاحب الحجز والمدير فقط.
      financeRejectedReason: mine ? b.financeRejectedReason : null,
      discountExceeded: b.discountExceeded,
      discountOverage: mine ? (dec(b.discountOverage) ?? 0) : 0,
      discountPercentAtBooking: mine ? dec(b.discountPercentAtBooking) : null,
      maxDiscountPercentAtBooking: mine ? dec(b.maxDiscountPercentAtBooking) : null,
      ...
      expectedCheckDate: mine ? b.expectedCheckDate : null,
      expectedTransferDate: mine ? b.expectedTransferDate : null,
      cashPaymentType: mine ? b.cashPaymentType : null,
      installmentsCount: mine ? b.installmentsCount : null,
