...
/** صلاحية كتابة المشاريع/الوحدات — مدير أو مدير مالي (قرار 2026-08-20)، بنفس نمط الرمي. */
async function requireProjectsWriter() {
  const user = await requireUser();
  if (!(user.role === "OWNER" || user.role === "ADMIN" || user.role === "FINANCE")) {
    throw new Error("إدارة المشاريع للمالك والمدير والمدير المالي فقط");
  }
  return user;
}
...
export async function createProject(formData: FormData): Promise<Result> {
  try {
    const user = await requireProjectsWriter();
